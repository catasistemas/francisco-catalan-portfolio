...
export default function PortfolioPage() {
  ...
  return (
    <main ref={shellRef} className="site-shell">
      ...
      <section className="numbers-band section-wrap" aria-label={t.statsLabel}>{t.stats.map((stat) => <div className="number-item" key={stat.value}><strong>{stat.value}</strong><span>{stat.line1}<br />{stat.line2}</span></div>)}</section>
